				...
				<div class="control-group">
					<div class="controls">
						<div class="main_input_box">
							<span class="add-on bg_lg">
							<i><img height="37" src="static/login/user.png" /></i>
							</span><input type="text" name="shopcode" id="shopcode" value="" placeholder="请输入商户号" />
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		function severCheck(){
			if(check()){
				...
				$.ajax({
					type: "POST",
					url: 'login_login',
			    	data: {KEYDATA:code,tm:new Date().getTime(),shopcode:shopcode},
					dataType:'json',
					cache: false,
					success: function(data){
						if("success" == data.result){
							...
							$("#code").focus();
						}else if("noshopcode" == data.result){
							$("#shopcode").tips({
								side : 1,
								msg : "不存在的商户号，请重新输入",
								bg : '#FF5080',
		...
		function check() {
			...
			if ($("#shopcode").val() == "") {
				...
				$("#shopcode").val(jQuery.trim($('#shopcode').val()));
			}
